...
public class Pawn extends Pieces implements Promoteable
{
	...
	public boolean EPpass(Board board, long coord, long EP) {				//checks if en passant leaves king open
		...
		if(team == 0) {
			EPpawn = (moves&EP)<<8;
			king = board.kingBB[0];
			rooks = board.rooksBB[1];
			queens = board.queensBB[1];
		}
		else {
			EPpawn = (moves&EP)>>8;
			king = board.kingBB[1];
			rooks = board.rooksBB[0];
			queens = board.queensBB[0];
		}
		long temp = coord|EPpawn;
		
		if((Moves.horizontal(~board.empty^temp, king)&(rooks|queens)) != 0)
			return false;
		else
			return true;
	}
